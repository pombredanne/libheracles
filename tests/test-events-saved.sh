...
run_heratool() {
savemode=$1
heratool --nostdinc -r $root -I $abs_top_srcdir/lenses <<EOF
set /heracles/save $savemode
...
for savemode in overwrite backup newfile noop; do
    ...
    saved=$(run_heratool $savemode | grep ^/heracles/events/saved | cut -d ' ' -f 3 | sort | tr '\n' ' ')
    ...
    if [ "$saved" != "$exp" ]
    then
        echo "Unexpected entries in /heracles/events/saved:"
        echo "Expected: \"$exp\""
        echo "Actual:   \"$saved\""
        echo "Save mode: $savemode"
        exit 1
