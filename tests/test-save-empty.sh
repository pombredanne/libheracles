...
(heratool $opts | grep ^/heracles) <<EOF
set /files/etc/hosts/1/ipaddr 127.0.0.1
set /files/etc/hosts/1/canonical localhost
save
print /heracles/files/etc/hosts/error
EOF
}

expected_errors() {
cat <<EOF
/heracles/files/etc/hosts/error = "mk_heratemp"
/heracles/files/etc/hosts/error/message = "No such file or directory"
EOF
}
