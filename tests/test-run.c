/*
 * test-run.c: test the hera_srun API function
 *
...
/* This array needs to be kept in sync with hera_errcode_t, and
...
static int load_module(struct heracles *hera, struct test *test) {
    char *fname;
    int r;
    ...
    if (asprintf(&fname, "%s/%s.hera", lensdir, test->module) == -1)
        fail(true, "asprintf test->module");
    for (int i=0; i < strlen(fname); i++)
        fname[i] = tolower(fname[i]);

    r = __hera_load_module_file(hera, fname);
...
static int run_one_test(struct test *test) {
    ...
    hera = hera_init("/dev/null", lensdir, HERA_NO_STDINC|HERA_NO_MODL_AUTOLOAD);
    fail(hera == NULL, "hera_init");
    fail(hera_error(hera) != HERA_NOERROR, "hera_init: errcode was %d",
         hera_error(hera));

    printf("%-30s ... ", test->name);

    r = load_module(hera, test);
    if (r < 0)
        goto error;
    ...
    r = hera_srun(hera, ms.stream, test->cmd);
    fail(r != test->result, "return value: expected %d, actual %d",
         test->result, r);
    fail(hera_error(hera) != test->errcode, "errcode: expected %s, actual %s",
         errtokens[test->errcode], errtokens[hera_error(hera)]);
