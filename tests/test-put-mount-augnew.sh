...
# Test that we can write into a bind mount placed at PATH.heranew with the
...
ROOT=$abs_top_builddir/build/test-put-mount-heranew
...
touch $TARGET $HOSTS_HERANEW

mount --bind $TARGET $HOSTS_HERANEW
Exit() {
    umount $HOSTS_HERANEW
    exit $1
}

HOSTS_SUM=$(sum $HOSTS)

heratool --nostdinc -I $LENSES -r $ROOT --new <<EOF
...
if [ ! -s $HOSTS_HERANEW ]; then
    echo "/etc/hosts.heranew is empty"
    Exit 1
fi
if [ ! -s $TARGET ]; then
