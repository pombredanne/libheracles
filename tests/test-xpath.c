...
static int run_one_test(struct heracles *hera, struct test *t) {
    int nexp = 0, nact;
    char **matches;
    int result = 0;
    ...
    list_for_each(e, t->entries)
        nexp++;
    nact = hera_match(hera, t->match, &matches);
    if (nact != nexp) {
        result = -1;
    } else {
        struct entry *e;
    ...
    if (result == 0) {
        ...
        if (nact < 0) {
            printf("  Actual: hera_match failed\n");
            } else {
            printf("  Actual: %d entries\n", nact);
        }
        for (int i=0; i < nact; i++) {
            const char *val;
            hera_get(hera, matches[i], &val);
...
static int test_rm_var(struct heracles *hera) {
    int r;

    printf("%-30s ... ", "rm_var");
    r = hera_defvar(hera, "h", "/files/etc/hosts/2/ipaddr");
    if (r < 0)
        die("hera_defvar failed");

    r = hera_match(hera, "$h", NULL);
    if (r != 1) {
        fprintf(stderr, "expected 1 match, got %d\n", r);
        goto fail;
    }

    r = hera_rm(hera, "/files/etc/hosts/2");
    if (r != 4) {
...
static int test_defvar_nonexistent(struct heracles *hera) {
    int r;

    printf("%-30s ... ", "defvar_nonexistent");
    r = hera_defvar(hera, "x", "/foo/bar");
    if (r < 0)
        die("hera_defvar failed");

    r = hera_set(hera, "$x", "baz");
    if (r != -1)
        goto fail;
    printf("PASS\n");
    return 0;
...
static int test_defnode_nonexistent(struct heracles *hera) {
    int r, created;

    printf("%-30s ... ", "defnode_nonexistent");
    r = hera_defnode(hera, "x", "/defnode/bar[0 = 1]", "foo", &created);
    if (r != 1)
        die("hera_defnode failed");
    if (created != 1) {
        fprintf(stderr, "defnode did not create a node\n");
        goto fail;
    }
    r = hera_match(hera, "$x", NULL);
    if (r != 1) {
        fprintf(stderr, "$x must have exactly one entry, but has %d\n", r);
        goto fail;
    }

    r = hera_defnode(hera, "x", "/defnode/bar", NULL, &created);
    if (r != 1)
        die("hera_defnode failed");
    if (created != 0) {
...
static int test_invalid_regexp(struct heracles *hera) {
    int r;

    printf("%-30s ... ", "invalid_regexp");
    r = hera_match(hera, "/files/*[ * =~ regexp('.*[aeiou')]", NULL);
    if (r >= 0)
        goto fail;
...
static int test_wrong_regexp_flag(struct heracles *hera) {
    int r;

    printf("%-30s ... ", "wrong_regexp_flag");
    r = hera_match(hera, "/files/*[ * =~ regexp('abc', 'o')]", NULL);
    if (r >= 0)
        goto fail;
...
static int run_tests(struct test *tests, int argc, char **argv) {
    ...
    hera = hera_init(root, lensdir, HERA_NO_STDINC|HERA_SAVE_NEWFILE);
    if (hera == NULL)
        die("hera_init");
    r = hera_defvar(hera, "hosts", "/files/etc/hosts/*");
    if (r != 6)
        die("hera_defvar $hosts");
    r = hera_defvar(hera, "localhost", "'127.0.0.1'");
    if (r != 0)
        die("hera_defvar $localhost");
    r = hera_defvar(hera, "php", "/files/etc/php.ini");
    if (r != 1)
        die("hera_defvar $php");
    ...
    list_for_each(t, tests) {
        if (! should_run(t->name, argc, argv))
            continue;
        if (run_one_test(hera, t) < 0)
            result = EXIT_FAILURE;
    }

    if (argc == 0) {
        if (test_rm_var(hera) < 0)
            result = EXIT_FAILURE;

        if (test_defvar_nonexistent(hera) < 0)
            result = EXIT_FAILURE;

        if (test_defnode_nonexistent(hera) < 0)
            result = EXIT_FAILURE;

        if (test_invalid_regexp(hera) < 0)
            result = EXIT_FAILURE;

        if (test_wrong_regexp_flag(hera) < 0)
            result = EXIT_FAILURE;
    }
    hera_close(hera);
