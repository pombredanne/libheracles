...
heratool --nostdinc -I $LENSES -b -r $ROOT > /dev/null <<EOF
set /files/etc/hosts/1/alias myhost
save
EOF

HOSTS_HERASAVE=${HOSTS}.herasave
if [ ! -f $HOSTS_HERASAVE ] ; then
    echo "Missing /etc/hosts.herasave"
    exit 1
fi
if [ -h $HOSTS_HERASAVE ] ; then
    echo "The file /etc/hosts.herasave is a symlink"
    exit 1
fi
if [ ! -h $HOSTS ] ; then
