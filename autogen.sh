...
(autoconf --version) < /dev/null > /dev/null 2>&1 || {
	echo
	echo "You must have autoconf installed to compile heracles."
...
(automake --version) < /dev/null > /dev/null 2>&1 || {
	echo
	DIE=1
	echo "You must have automake installed to compile heracles."
	echo "Download the appropriate package for your distribution,"
...
$srcdir/configure "${CF_ARGS[@]}" && {
    echo
    echo "Now type 'make' to compile heracles."
}
