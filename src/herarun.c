/*
 * herarun.c: command interpreter for heracles
 *
...
struct command {
    ...
    struct error             *error; /* Same as hera->error */
    FILE                     *out;
    bool                      quit;
};
...
static char *nexttoken(struct command *cmd, char **line, bool path) {
    ...
    while (*s) {
        ...
        if (*s == '\\') {
            switch (*(s+1)) {
                ...
                case '\\': /* pass next character through */
                    ...
                    break;
                default:
                    ERR_REPORT(cmd, HERA_ECMDRUN, "unknown escape sequence");
                    return NULL;
            }
        }
        ...
        if (nescaped == 0) {
            ...
            if (*s == ']') nbracket -= 1;
            if (nbracket < 0) {
                ERR_REPORT(cmd, HERA_ECMDRUN, "unmatched [");
                return NULL;
            }
    ...
    if (*s == '\0' && quot) {
        ERR_REPORT(cmd, HERA_ECMDRUN, "unmatched %c", quot);
        return NULL;
    }
    while (*w && w <= s)
        *w++ = '\0';
...
static int parseline(struct command *cmd, char *line) {
    ...
    if (cmd->def == NULL) {
        ERR_REPORT(cmd, HERA_ECMDRUN, "Unknown command '%s'", tok);
        return -1;
    }
    ...
    if (curarg < narg - nopt) {
        ERR_REPORT(cmd, HERA_ECMDRUN, "Not enough arguments for %s", cmd->def->name);
        return -1;
    }
...
static int child_count(struct command *cmd, const char *path) {
    ...
    if (q == NULL)
        return 0;
    cnt = hera_match(cmd->hera, q, NULL);
    if (HAS_ERR(cmd))
        cnt = -1;
    free(q);
    return cnt;
...
static void cmd_match(struct command *cmd) {
    ...
    cnt = hera_match(cmd->hera, pattern, &matches);
    ERR_BAIL(cmd);
    ERR_THROW(cnt < 0, cmd->hera, HERA_ECMDRUN,
              "  (error matching %s)\n", pattern);
    if (cnt == 0) {
    ...
    for (int i=0; i < cnt; i++) {
        const char *val;
        hera_get(cmd->hera, matches[i], &val);
        ERR_BAIL(cmd);
        if (val == NULL)
            val = "(none)";
        if (filter) {
...
static void cmd_rename(struct command *cmd) {
    ...
    cnt = hera_rename(cmd->hera, src, lbl);
    if (cnt < 0)
        ERR_REPORT(cmd, HERA_ECMDRUN,
...
static void cmd_set(struct command *cmd) {
    ...
    r = hera_set(cmd->hera, path, val);
    if (r < 0)
        ERR_REPORT(cmd, HERA_ECMDRUN, "Setting %s failed", path);
}
...
static void cmd_setm(struct command *cmd) {
    ...
    hera_setm(cmd->hera, base, sub, val);
}
...
static void cmd_clearm(struct command *cmd) {
    ...
    hera_setm(cmd->hera, base, sub, NULL);
}
...
static void cmd_span(struct command *cmd) {
    ...
    if (hera_get(cmd->hera, HERACLES_SPAN_OPTION, &option) != 1) {
        printf("Error: option " HERACLES_SPAN_OPTION " not found\n");
        return;
    }
    if (streqv(HERA_DISABLE, option)) {
        ERR_REPORT(cmd, HERA_ECMDRUN,
                   "Span is not enabled. To enable, run the commands:\n"
                   "    set %s %s\n    rm %s\n    load\n",
                   HERACLES_SPAN_OPTION, HERA_ENABLE, HERACLES_FILES_TREE);
        return;
    } else if (! streqv(HERA_ENABLE, option)) {
        ERR_REPORT(cmd, HERA_ECMDRUN,
                   "option %s must be %s or %s\n", HERACLES_SPAN_OPTION,
                   HERA_ENABLE, HERA_DISABLE);
        return;
    }
    r = hera_span(cmd->hera, path, &filename, &label_start, &label_end,
                 &value_start, &value_end, &span_start, &span_end);
    ERR_THROW(r == -1, cmd, HERA_ECMDRUN, "failed to retrieve span");
...
static void cmd_defvar(struct command *cmd) {
    ...
    hera_defvar(cmd->hera, name, path);
}
...
static void cmd_defnode(struct command *cmd) {
    ...
    if (value != NULL && strlen(value) == 0)
        value = NULL;
    hera_defnode(cmd->hera, name, path, value, NULL);
}
...
static void cmd_clear(struct command *cmd) {
    ...
    r = hera_set(cmd->hera, path, NULL);
    if (r < 0)
        ERR_REPORT(cmd, HERA_ECMDRUN, "Clearing %s failed", path);
}
...
static void cmd_touch(struct command *cmd) {
    ...
    r = hera_match(cmd->hera, path, NULL);
    if (r == 0) {
        r = hera_set(cmd->hera, path, NULL);
        if (r < 0)
            ERR_REPORT(cmd, HERA_ECMDRUN, "Touching %s failed", path);
    }
}
...
static void cmd_label(struct command *cmd) {
    ...
    r = hera_label(cmd->hera, path, &lbl);
    ERR_RET(cmd);
    fprintf(cmd->out, "%s", path);
    if (r == 0) {
        fprintf(cmd->out, " (o)\n");
...
static void cmd_print(struct command *cmd) {
    ...
    hera_print(cmd->hera, cmd->out, path);
}
...
static void cmd_transform(struct command *cmd) {
    ...
    r = hera_transform(cmd->hera, lens, file, excl);
    if (r < 0)
        ERR_REPORT(cmd, HERA_ECMDRUN,
...
static void cmd_save(struct command *cmd) {
    int r;
    r = hera_save(cmd->hera);
    if (r == -1) {
        ERR_REPORT(cmd, HERA_ECMDRUN,
                   "saving failed (run 'print /heracles//error' for details)");
    } else {
        r = hera_match(cmd->hera, "/heracles/events/saved", NULL);
        if (r > 0) {
...
static const struct command_def cmd_load_def = {
    ...
    .help = "Load files  according to the  transforms in /heracles/load.  "
    "A transform\n Foo  is  represented  with  a  subtree  /heracles/load/Foo."
    "   Underneath\n /heracles/load/Foo, one node labelled  'lens' must exist,"
...
static void cmd_ins(struct command *cmd) {
    ...
    hera_insert(cmd->hera, path, label, before);
}
...
static void cmd_store(struct command *cmd) {
    ...
    hera_text_store(cmd->hera, lens, node, path);
}
...
static void cmd_retrieve(struct command *cmd) {
    ...
    hera_text_retrieve(cmd->hera, lens, node_in, path, node_out);
}
...
static void cmd_help(struct command *cmd) {
    ...
    if (name == NULL) {
        ...
        ERR_THROW(def == NULL, cmd->hera, HERA_ECMDRUN,
                  "unknown command %s\n", name);
