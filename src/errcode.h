...
#include "internal.h"
/* Include heracles.h for the error codes */
#include "heracles.h"
...
void report_error(struct error *err, hera_errcode_t errcode,
                  const char *format, ...)
...
#define HAS_ERR(obj) ((obj)->error->code != HERA_NOERROR)

#define ERR_BAIL(obj) if ((obj)->error->code != HERA_NOERROR) goto error;

#define ERR_RET(obj) if ((obj)->error->code != HERA_NOERROR) return;

#define ERR_NOMEM(cond, obj)                             \
    if (cond) {                                          \
        report_error((obj)->error, HERA_ENOMEM, NULL);    \
        goto error;                                      \
    }
...
#define ARG_CHECK(cond, obj, fmt ...)                           \
    do {                                                        \
        if (cond) {                                             \
            report_error((obj)->error, HERA_EBADARG, ## fmt);    \
            goto error;                                         \
        }                                                       \
    } while(0)
