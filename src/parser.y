...
# define YYLLOC_DEFAULT(Current, Rhs, N)                                \
  do {                                                                  \
    (Current).filename = heral_get_info(scanner)->filename;              \
    (Current).error = heral_get_info(scanner)->error;                    \
    if (YYID (N)) {                                                     \
...
%initial-action {
  ...
  @$.last_line    = 1;
  @$.last_column  = 0;
  @$.filename     = heral_get_info(scanner)->filename;
  @$.error        = heral_get_info(scanner)->error;
};
...
%{
/* Lexer */
extern int heral_lex (YYSTYPE * yylval_param,struct info * yylloc_param ,yyscan_t yyscanner);
int heral_init_lexer(struct state *state, yyscan_t * scanner);
void heral_close_lexer(yyscan_t *scanner);
int heral_lex_destroy (yyscan_t yyscanner );
int heral_get_lineno (yyscan_t yyscanner );
int heral_get_column  (yyscan_t yyscanner);
struct info *heral_get_info(yyscan_t yyscanner);
char *heral_get_text (yyscan_t yyscanner );

static void heral_error(struct info *locp, struct term **term,
...
int heral_parse_file(struct heracles *hera, const char *name,
                    struct term **term) {
  yyscan_t          scanner;
  struct state      state;
...
void heral_error(struct info *locp,
                struct term **term,
                yyscan_t scanner,
                const char *s) {
  struct info info;
  ...
  if (locp != NULL) {
    ...
    info.first_line   = heral_get_lineno(scanner);
    info.first_column = heral_get_column(scanner);
    info.last_line    = heral_get_lineno(scanner);
    info.last_column  = heral_get_column(scanner);
    info.filename     = heral_get_info(scanner)->filename;
    info.error        = heral_get_info(scanner)->error;
