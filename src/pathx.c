...
struct state {
    ...
    /* Error structure, used to communicate errors to struct heracles;
...
static void store_error(struct pathx *pathx) {
    ...
    if (err == NULL || errcode == PATHX_NOERROR || err->code != HERA_NOERROR)
        return;

    switch (errcode) {
    case PATHX_ENOMEM:
        err->code = HERA_ENOMEM;
        break;
    case PATHX_EMMATCH:
        err->code = HERA_EMMATCH;
        break;
    case PATHX_ENOMATCH:
        err->code = HERA_ENOMATCH;
        break;
    default:
        err->code = HERA_EPATHX;
        break;
    }

    /* We only need details for pathx syntax errors */
    if (err->code != HERA_EPATHX)
        return;
