...
static void vreport_error(struct error *err, hera_errcode_t errcode,
                   const char *format, va_list ap) {
    /* We only remember the first error */
    if (err->code != HERA_NOERROR)
        return;
...
void bug_on(struct error *err, const char *srcfile, int srclineno,
    ...
    va_start(ap, format);
    vreport_error(err, HERA_EINTERNAL, format, ap);
    va_end(ap);
