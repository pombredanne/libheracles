...
/* Load all files matching the TRANSFORM's filter into the tree in HERA by
 ...
 * about filename underneath "/heracles/files" + filename
 */
 ...
 */
int text_store(struct heracles *hera, const char *lens_name,
...
/* Remove the file for TREE, either by moving it to a .herasave file or by
 * unlinking it, depending on hera->flags. TREE must be the node underneath
 * /heracles/files corresponding to the file to be removed.
 *
...
/* Store a file-specific transformation error in /heracles/files/PATH/error */
