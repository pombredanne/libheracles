...
static void format_error(struct info *info, hera_errcode_t code,
...
void syntax_error(struct info *info, const char *format, ...) {
    ...
    if (error->code != HERA_NOERROR && error->code != HERA_ESYNTAX)
        return;

	va_start(ap, format);
    format_error(info, HERA_ESYNTAX, format, ap);
    va_end(ap);
}
...
void fatal_error(struct info *info, const char *format, ...) {
    ...
    if (error->code == HERA_EINTERNAL)
        return;

	va_start(ap, format);
    format_error(info, HERA_EINTERNAL, format, ap);
    va_end(ap);
}
 ...
 * Modules
 */
static int load_module(struct heracles *hera, const char *name);
...
static int lookup_internal(struct heracles *hera, const char *ctx_modname,
    ...
    *bnd = NULL;
    ...
    if (modname == NULL) {
        struct module *builtin =
            module_find(hera->modules, builtin_module);
        assert(builtin != NULL);
 ...
 qual_lookup:
    list_for_each(module, hera->modules) {
    ...
    int loaded = load_module(hera, modname) == 0;
    if (loaded)
        goto qual_lookup;
...
struct lens *lens_lookup(struct heracles *hera, const char *qname) {
    struct binding *bnd = NULL;

    if (lookup_internal(hera, NULL, qname, &bnd) < 0)
        return NULL;
...
static struct binding *ctx_lookup_bnd(struct info *info,
    ...
    if (ctx->hera != NULL) {
        int r;
        r = lookup_internal(ctx->hera, ctx->name, name, &b);
        if (r == 0)
            return b;
        char *modname = modname_of_qname(name);
...
static void dump_ctx(struct ctx *ctx) {
    ...
    if (ctx->hera != NULL) {
        list_for_each(m, ctx->hera->modules)
            dump_module(m);
    }
}
...
static int typecheck(struct term *term, struct heracles *hera) {
    int ok = 1;
    struct ctx ctx;
    char *fname;
    const char *basenam;
...
static struct module *compile(struct term *term, struct heracles *hera) {
    struct ctx ctx;
...
/* Defined in parser.y */
int heral_parse_file(struct heracles *hera, const char *name, struct term **term);

static char *module_basename(const char *modname) {
    char *fname;

    if (asprintf(&fname, "%s" HERA_EXT, modname) == -1)
        return NULL;
...
static char *module_filename(struct heracles *hera, const char *modname) {
    char *dir = NULL;
    char *filename = NULL;
...
int load_module_file(struct heracles *hera, const char *filename) {
    struct term *term = NULL;
    int result = -1;

    if (hera->flags & HERA_TRACE_MODULE_LOADING)
        printf("Module %s", filename);
    heral_parse_file(hera, filename, &term);
    if (hera->flags & HERA_TRACE_MODULE_LOADING)
        printf(HAS_ERR(hera) ? " failed\n" : " loaded\n");
    ERR_BAIL(hera);

    if (! typecheck(term, hera))
        goto error;

    struct module *module = compile(term, hera);
    ERR_THROW(module == NULL, hera, HERA_ESYNTAX,
              "Failed to load %s", filename);

    list_append(hera->modules, module);
    result = 0;
 error:
    // FIXME: This leads to a bad free of a string used in a del lens
    // To reproduce run lenses/tests/test_yum.hera
    unref(term, term);
    return result;
}
...
static int load_module(struct heracles *hera, const char *name) {
    ...
    if (module_find(hera->modules, name) != NULL)
        return 0;

    if ((filename = module_filename(hera, name)) == NULL)
        return -1;

    if (load_module_file(hera, filename) == -1)
        goto error;
...
int interpreter_init(struct heracles *hera) {
    int r;

    r = init_fatal_exn(hera->error);
    if (r < 0)
        return -1;

    hera->modules = builtin_init(hera->error);
    if (hera->flags & HERA_NO_MODL_AUTOLOAD)
        return 0;
    ...
    while ((dir = argz_next(hera->modpathz, hera->nmodpath, dir)) != NULL) {
        ...
        if (r != 0 && r != GLOB_NOMATCH) {
            ...
            hera_errcode_t code =
                r == GLOB_NOSPACE ? HERA_ENOMEM : HERA_EINTERNAL;
            ERR_REPORT(hera, code, "glob failure for %s", globpat);
            free(globpat);
            goto error;
        }
