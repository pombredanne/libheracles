...
heraparse - execute an Augeas module

=head1 SYNOPSIS

heraparse [OPTIONS] MODULE
...
HERACLES_LENS_LIB environment variable, and before the default directory
F</usr/share/heracles/lenses>.
...
To run the tests in F<lenses/tests/test_foo.hera> and use modules from the
directory F<lenses>, run

=over 4

heraparse -I lenses lenses/tests/test_foo.hera
...
tests for a module F<lenses/foo.hera> are kept in a separate file, usually
in F<lenses/tests/test_foo.hera>.
