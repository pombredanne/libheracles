...
heratool - inspect and modify configuration files

=head1 SYNOPSIS

heratool [OPTIONS] [COMMAND]
...
heratool provides a command line interface to the generated tree. COMMAND
...
Save changes in files with extension '.heranew', do not modify the original
files

=item B<-r>, B<--root>=I<ROOT>

Use directory ROOT as the root of the filesystem. Takes precedence over a
root set with the HERACLES_ROOT environment variable.
...
HERACLES_LENS_LIB environment variable, and before the default directories
F</usr/share/heracles/lenses> and F</usr/share/heracles/lenses/dist>.
...
B<HERACLES_LENS_LIB> will be searched for modules.

=item B<-L>, B<--noload>

Do not load any files on startup. This is generally used to fine-tune which
files to load by modifying the entries in C</heracles/load> and then issuing
a C<load> command.

=item B<-A>, B<--noautoload>

Do not load any lens modules, and therefore no files, on startup. This
creates no entries under C</heracles/load> whatsoever; to read any files,
...
Load files according to the transforms in C</heracles/load>.
...
You need to run 'set /heracles/span enable' prior to loading files to enable
...
but before the default directories F</usr/share/heracles/lenses> and
F</usr/share/heracles/lenses/dist>
...
in C</heracles/load> is not considered a failure. If it is important to know
that all files were loaded, you need to issue a C<match /heracles//error>
...
=head1 EXAMPLES
  ...
  # command line mode
  heratool print /files/etc/hosts/
  ...
  heratool print '/files/etc/ssh/sshd_config/AcceptEnv[2]/3'

  # Find the entry in inittab with action 'initdefault'
  heratool> match /files/etc/inittab/*/action initdefault

  # Print the last alias for each entry in /etc/hosts
  heratool> print /files/etc/hosts/*/alias[last()]

=head1 FILES

Lenses and schema definitions in F</usr/share/heracles/lenses> and
F</usr/share/heracles/lenses/dist>
...
Augeas (and heratool) are distributed under the GNU Lesser General Public
License (LGPL)

=head1 SEE ALSO

B<Augeas> project homepage L<http://www.heracles.net/>
